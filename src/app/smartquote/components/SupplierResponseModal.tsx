...
export default function SupplierResponseModal(
  ...
  const [productName, setProductName] = useState<string>(row?.response_product_name || "");
  ...
  useEffect(() => {
    if (!row) return;
    setProductName(row.response_product_name ?? ""); 
  ...
  const formatIDR = (n: number | null | undefined) => (!n || n <= 0 ? "" : `Rp ${n.toLocaleString("id-ID")}`);
  const parseIDR = (s: string) => { const d = s.replace(/[^\d]/g, ""); return d ? parseInt(d, 10) : 0; };
      ...
      <div className="w-full max-w-lg rounded-xl bg-white shadow-lg">
        ...
        <div className="p-4 space-y-3 text-sm">
          <div className="grid grid-cols-2 gap-3">
            <div><div className="text-gray-500">SQ ID</div><div className="font-medium">{row.sq_id}</div></div>
          ...
          <label className="block">
            ...
            <input className="mt-1 w-full rounded border px-2 py-1" value={productName}
                   onChange={(e) => setProductName(e.target.value)} placeholder="Nama barang yang Anda tawarkan" />
          </label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block">
              ...
              <input type="text" inputMode="numeric" className="mt-1 w-full rounded border px-2 py-1"
                     placeholder="Rp 0" value={formatIDR(price)}
                     onChange={(e) => setPrice(parseIDR(e.target.value))}
                     onBlur={() => { if (!price || price < 1) setPrice(1); }} />
            </label>
          </div>
          ...
          <label className="block">
            ...
            <input type="text" className="mt-1 w-full rounded border px-2 py-1"
                   value={note} onChange={(e) => setNote(e.target.value)} placeholder="Catatan untuk customer" />
          </label>
        </div>

        <div className="px-4 py-3 border-t flex items-center justify-end gap-2">
          <button onClick={onClose} className="px-3 py-1 rounded border hover:bg-gray-50" disabled={loading}>Cancel</button>
          <button onClick={submit} className="px-3 py-1 rounded bg-green-600 hover:bg-green-700 text-white disabled:opacity-50"
                  disabled={loading || !productName.trim() || !quantity || !price}>
